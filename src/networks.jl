...
function (s::Source)(; rows=:)
    if rows == (:)
        ...
        return retrieve(s.data, Rows, rows)
    end
end
...
# Allow nodes to share the `retrieve(X, Rows, r)` syntax of concrete tabular data
# (needed for `fit(::AbstractMachine, ...)` in machines.jl):
MLJBase.retrieve(X::AbstractNode, ::Type{Rows}, r) = X(rows=r)
