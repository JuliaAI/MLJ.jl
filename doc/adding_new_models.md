...
`evaluate`. But other operation names can be "registered" by adding a
line to "src/operations.jl".
...
`SomeSupervisedModel{R} <: Supervised{R}`. These are what go into package
...
models, should be returned in the `report` object. How or if these are
generated should be controlled by hyperparameters. The `report` object
returned is either a `Dict{Symbol,Any}` object, or `nothing` if there
is nothing to report. So for example, `fit` might declare
...
`fit` type signature. Checks not specific to the package (e.g.,
...
MLJ.operations(::Type{DecisionTreeClassifier}) = (MLJ.predict,) # TODO: add MLJ.predict_proba
...
Generally `coerce` and `coerce_training` methods are required to
coerce generic data into the form required by the `predict` (and
`predict_proba`) and `fit` methods provided, respectively. At present,
the method `MLJ.array` will convert any tabular data container that MLJ plans
to support into an `Array` and a common choice for coercion will be

````julia
MLJ.coerce(model::SomeModelType, X) = (MLJ.array(X),)
MLJ.coerce_training(model::SomeModelType, X, y) = (MLJ.array(X), collect(y)]) 
````

for classifiers (`y` being a `CategoricalArray`). For regressors, the
second definition will commonly be replaced with

````julia
MLJ.coerce_training(model::SomeModelType, X, y) = (MLJ.array(X), Float64[y...]) 
````

The fallback definitions of `coerce` and `coerce_training` perform no
coercion (are identity functions).
...
should return a `Vector{Float64}` of probabilities (one probability
per input pattern).
...
`Array{Float64}` object of size `(nrows, k)` where `nrows` is the
number of input patterns (the number of rows of the input data `Xnew`)
and `k` is the number of labels. In addition, a method `classes` must
be implemented such that `classes(fitresult)` is the vector of the
unique labels, ordered such that the probabilities in the `j`th column
of `yhat` correspond to the `j`th label.
...
training data) to avoid repeating computations unnecessarily. (A
fallback just calls `fit`.)  Composite models (subtypes of
`Supervised{Node}`) constitute one use-case (component models
are only retrained when new hyperparameter values make this necessary)
and in this case MLJ provides a fallback. A second important use-case
is iterative models, where calls to increase the number of iterations
only restarts the iterative procedure if other hyperparameters have
also changed. For an example see `builtins/Ensembles.jl`.
