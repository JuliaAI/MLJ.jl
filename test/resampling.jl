...
result = @notest_logs((:info, r"^Evaluating using a holdout set"),
                    evaluate!(mach, resampling=holdout, measure=[rms, rmslp1]))
@test result isa NamedTuple
@test @notest_logs((:info, r"^Evaluating using a holdout set"),
...
model = ConstantRegressor()
mach = machine(model, X, y)
result = @notest_logs((:info, r"^Evaluating using cross-validation"),
                    evaluate!(mach, resampling=cv, measure=[rms, rmslp1]))
@test result isa NamedTuple
errs = @notest_logs((:info, r"^Evaluating using cross-validation"),
                  evaluate!(mach, resampling=cv))
for e in errs
    @test e ≈ 1/2 || e ≈ 3/4
end
@test scitype_union(@notest_logs((:info, r"^Evaluating using cross-validation"),
...
@notest_logs((:info, r"^Training"), fit!(resampling_machine))
e1=evaluate(resampling_machine)
mach = machine(ridge_model, X, y)
e1 ≈ @notest_logs((:info, r"^Evaluating using a holdout set"),
                evaluate!(mach, holdout))
ridge_model.lambda=1.0
@notest_logs((:info, r"^Updating"), (:info, r"^Evaluating using a holdout set"),
...
e1=evaluate(resampling_machine)
mach = machine(ridge_model, X, y)
e1 ≈ @notest_logs((:info, r"^Evaluating using cross-validation"),
                evaluate!(mach, cv))
ridge_model.lambda=10.0
@notest_logs((:info, r"^Updating"), (:info, r"^Evaluating using cross-validation"),
           fit!(resampling_machine, verbosity=2))
e2=evaluate(resampling_machine)
